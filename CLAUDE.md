...
| `DNS_UPSTREAM` | `9.9.9.9` | Primary upstream DNS dnsmasq forwards to (written into generated dnsmasq.conf). Accepts `IP#port`. Currently `127.0.0.153` = local AdGuardDNSCLI daemon (see below) |
| `DNS_FALLBACK` | `149.112.112.112` | Fallback upstream DNS (written into generated dnsmasq.conf). A public IP here keeps DNS alive if the primary dies, but those queries bypass AdGuard filtering |
...
dnsmasq's default upstream can be the local **AdGuardDNSCLI** daemon, which forwards queries encrypted (DoH) to the user's hosted AdGuard DNS account — directly over WAN, no WireGuard dependency for DNS:

```
apps → dnsmasq (ipset tagging) → AdGuardDNSCLI (127.0.0.153:53) → https://d.adguard-dns.com/dns-query/<device_id>
```

- Install location (this machine): `/mnt/nvme2/games/backups/Downloads/AdGuardDNSCLI_linux_amd64/AdGuardDNSCLI/`
- Systemd unit: `AdGuardDNSCLI.service` (system-level; restart needs sudo)
- Config: `config.yaml` **next to the binary** — there is no `-c` flag; the binary always reads its own directory
- No config reload — edits require `sudo systemctl restart AdGuardDNSCLI`
- `dns.server.listen_addresses` must be only `127.0.0.153:53`. The installer auto-generates a listen entry for every interface IP, which crash-loops on any occupied port 53 (k3s, libvirt) and would collide with dnsmasq on `172.17.0.1:53`
- `dns.upstream.groups.default.address` holds the per-device DoH URL from the AdGuard DNS dashboard (DoT/DoQ addresses for the same device also work)
- Wire-up: `DNS_UPSTREAM=127.0.0.153` in `config.env`, then re-run `proxy-on.sh`
- Debugging: copy binary + `config.yaml` to a scratch dir, change listen to a high port, set `log.output: stdout` and `log.verbose: true`, run in foreground
...
proxy-on.sh
  ...
  |-- locks: ~/.proxy-firewall-baseline/ directory (flock, shared with proxy-off.sh)
...
proxy-off.sh
  |-- sources: config.env
  |-- locks: ~/.proxy-firewall-baseline/ (flock); removes: enabled marker (first, so the watcher stops repairing)
...
proxy-watch.sh  [long-running, proxy-watch.service; or --once]
  ...
  |-- requires: root or passwordless sudo (exits 1 otherwise); shipped unit runs as root
...
sudo journalctl -u proxy-watch -f
sudo ~/routekeeper/scripts/proxy-watch.sh --once   # manual check + repair
```
...
11. **Enabled marker + lock**: `proxy-on.sh` touches `$BASELINE_DIR/enabled` after installing rules; `proxy-off.sh` removes it first. Both hold `flock` on the `$BASELINE_DIR` directory itself (no lock file, so root- and user-run invocations never hit an ownership clash). `proxy-watch.sh` only repairs while the marker exists and calls `proxy-on.sh --if-enabled` (re-checks the marker under the lock), so it can never re-enable a proxy the user disabled. Kernel state (ipsets, iptables) and both containers survive a WireGuard drop; the usual real drift is `$IFACE` DNS reset by a WiFi reconnect, or the `wg0` default route in gateway mode. If you add new state to `proxy-on.sh`, add a matching check to `find_problem()` in `proxy-watch.sh`.
